...
$$
25^{th} percentile = \mu_{0}-\tau_{0} = 125-5 = 120
$$
$$
25^{th} percentile = \mu_{0}+\tau_{0} = 125+5 = 130
$$

Gaussian Normal Distribution:
25th and 75th quantile each are 1 standard deviation left and right from the mean containing 68% of the probability mass. 
...
$$
\sigma^{2} \left(\frac{1}{\tau_{n}^2} - \frac{1}{\tau_{0}^2}\right) = N \\
\tau_{0} = 5 \\
$$
$$
\tau_n = 2;\space 25*\left(\frac{1}{4} - \frac{1}{25}\right) ; N = 5.25   \\
$$

$$
\tau_n =0.5;\space 25 *\left(\frac{1}{.25} - \frac{1}{25}\right) ; N = 99
$$
...
You may add as many equation blocks and code chunks as you see fit to answer this question.  
$$
\sigma^{2} \left(\frac{1}{\tau_{n}^2} - \frac{1}{\tau_{0} \rightarrow \infty}\right) = N \\
$$
$$
\tau_{0} = 5
\\
...
$$
$$
225 * \left(\frac{1}{9} - 0\right) = N = 25
$$
...
$$
f'(u) =  \frac{1}{ \sigma^2} \Sigma_{n=1}^{N} \left(   x_n  \right) - \Sigma_{n=1}^{N} \left(   \mu \right)- \frac{1}{ \tau_{0}^2}(\mu -\mu_0) 
$$

$$
f'(u) =  \frac{N \bar{x}}{\sigma^2}  - \mu N - \frac{\mu -\mu_0}{ \tau_{0}^2}
$$
...
$$
0 =  \frac{N \bar{x}}{\sigma^2}  - \mu N - \frac{\mu -\mu_0}{ \tau_{0}^2} 
$$

$$
0 =  N \bar{x} \tau_{0}^2  - \mu N \tau_{0}^2 \sigma^2 - \mu\sigma^2  +\mu_0 \sigma^2 
$$

$$
-N \bar{x} \tau_{0}^2  - \mu_0 \sigma^2 = - \mu N \tau_{0}^2 \sigma^2 - \mu\sigma^2 
$$
...
$$
N \bar{x} \tau_{0}^2  + \mu_0 \sigma^2 =  \mu( N \tau_{0}^2 \sigma^2 + \sigma^2) 
$$

$$
\frac{N \bar{x} \tau_{0}^2  + \mu_0 \sigma^2}{N \tau_{0}^2 \sigma^2 + \sigma^2} =  \mu_{MAP} 
$$
...
The posterior  is a normal distribution resulting since the normal prior is conjugate to the normal distribution of likelihood. 

The resulting expression is equivalent to the posterior mean expression from maximizing the likelihood through the derivation. 
...
$$
\log \left( p \left(\sigma \mid \mathbf{x}, \mu\right) \right) \propto \log \left( \prod_{n=1}^{N} \left( \frac{1}{\sigma\sqrt{2\pi}}\exp \left( - \frac{1}{2 \sigma^2}\left(   x_n - \mu \right)^2\right) \right) \times  \lambda \exp(-\lambda\sigma) \right) 
$$

$$
\log \left( p \left(\sigma \mid \mathbf{x}, \mu\right) \right) \propto\Sigma_{n=1}^{N}\log \left( \frac{1}{\sigma\sqrt{2\pi}} \exp \left( - \frac{1}{2 \sigma^2}\left(   x_n - \mu \right)^2\right) \right) + \log(\lambda\exp(-\lambda\sigma)) \\
$$

$$
\log \left( p \left(\sigma \mid \mathbf{x}, \mu\right) \right) \propto \Sigma_{n=1}^{N}\left( \log \left( \frac{1}{\sigma \sqrt{2\pi}}\right) - \frac{(x_n - \mu)^2}{2\sigma^2} \right) + \log(\lambda) + \log(\exp(-\lambda\sigma)) 
$$

$$
\log \left( p \left(\sigma \mid \mathbf{x}, \mu\right) \right) \propto\Sigma_{n=1}^{N}\left( \log \left( 1\right)  - \log( \sigma \sqrt(2\pi) - \frac{(x_n - \mu)^2}{2\sigma^2}\right) -\lambda\sigma
$$

$$
\log \left( p \left(\sigma \mid \mathbf{x}, \mu\right) \right) \propto\Sigma_{n=1}^{N}\left(  - (\log( \sigma) + \log(\sqrt2\pi))  - \frac{(x_n - \mu)^2}{2\sigma^2}\right) -\lambda\sigma
$$

$$
\log \left( p \left(\sigma \mid \mathbf{x}, \mu\right) \right) \propto\Sigma_{n=1}^{N}\left(  - (\log( \sigma)   - \frac{(x_n - \mu)^2}{2\sigma^2}\right) -\lambda\sigma
$$
...
$$
\log \left( p \left(\sigma \mid \mathbf{x}, \mu\right) \right) \propto N\left(  - (\log( \sigma)   - \frac{v}{2\sigma^2}\right) -\lambda\sigma\\
$$


$$
\log \left( p \left(\sigma \mid \mathbf{x}, \mu\right) \right) \propto -N \log( \sigma)   - \frac{N\times v}{2\sigma^2} -\lambda\sigma\\
$$
v sufficiently represents the unknown parameter and therefore the un-normalized posterior does not provided additional beneficial information to our assessment based on individual observations.
### 3c)
...
{
  # your code here
  result <- ((-N) * log(sigma)) - ((N*v)/(2*sigma**2)) - (lambda*sigma)
...
It has one mode and the curve is parabolic.
...
$$
\log \left( p_{\sigma} \left( g^{-1} \left(\varphi \right) \mid \mathbf{x}, \mu \right) \right)=\log \left( p \left(\sigma \mid \mathbf{x}, \mu\right) \right) \propto -N \log( \sigma)   - \frac{N\times v}{2\sigma^2} -\lambda\sigma\\
$$

$$
\log\left(p_{\varphi} \left(\varphi \mid \mathbf{x}, \mu\right) \right) = -N \log( \sigma)   - \frac{N\times v}{2\sigma^2} -\lambda\sigma + \log \left( \left| \frac{d}{d\varphi} \left( \exp(\varphi) \right) \right| \right) \\
$$

$$
\log\left(p_{\varphi} \left(\varphi \mid \mathbf{x}, \mu\right) \right) = -N \log( \sigma)   - \frac{N\times v}{2\sigma^2} -\lambda\sigma + \log \left( \left|  \exp(\varphi) \right| \right) \\
$$
$$
\log\left(p_{\varphi} \left(\varphi \mid \mathbf{x}, \mu\right) \right) = -N \log( \sigma)   - \frac{N\times v}{2\sigma^2} -\lambda\sigma + \varphi \\
$$

The inverse link function is another way to denote sigma.
...
log_post_varphi_unnorm <- function(varphi, N, v, lambda)
{
  ...
  # calculate the normalized log-posterior on sigma
  log_post <- ((-N) * log(sigma)) - ((N*v)/(2*sigma**2)) - (lambda*sigma)
  
  # account for the derivative adjustment
  result <- log_post + varphi
...
Yes the curve is more parabolic now.
...
The fnscale set to -1 makes the optim minimize instead of maximize, and gr should be set to NULL.
...
No the posterior mode found is the same regardless of initial guess.
...
```{r, solution_5c}
tibble::tibble(
  varphi = seq(-0.5, 2, length.out = 101)
) %>% 
  mutate(log_post = log_post_varphi_unnorm(varphi, N, v, 0.75)) %>%
  ggplot(mapping = aes(x = varphi, y = log_post)) +
  geom_line() +
