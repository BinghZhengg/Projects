---
title: "CS 1675 Spring 2022: Homework 11"
...
Yes, the distribution of the input values is mostly concentrated in the middle and the tails have more concise distributions.

```{r, solution_01a}
###
lf_sonar %>% ggplot() +
  geom_boxplot(aes(x = input_id, y = value, group = input_id))
```
...
```{r, solution_01b}
lf_sonar %>% ggplot(aes(x = value)) +
  geom_freqpoly(aes(group = input_id), bins = 21) +
...
Yes, the log transformation pre-processing would be appropriate as it would adjust the skew.
...
The middle and lower half of the visualizations exhibit notable clusters even if there is little correlation overall.

#### SOLUTION

```{r, solution_01d}
Sonar %>% select(-Class) %>% cor() %>% corrplot::corrplot(method = "square", order = "hclust", hclust.method = 'ward.D2')
```
...
Since the proportion of event to nonevent is similar to M to R, we shouldn't be concerned.
...
ctrl_k05_roc <- trainControl(method = "repeatedcv",
                             number = 5,
                             repeats = 3,
...
Yes preprocessing with the regularized model will penalize the large coefficients. This would affect the test set predictions, since it may lead to overfitting if we do not preprocess the training set.

**Based on the training results, does the elastic net model favor LASSO or RIDGE more?**  

The elastic model is more like ridge since the lambda is closer to 0.
...
The neutal network has a higher ROC

preprocessing is recommended to neural networks since the differences in the scales across inputs makes models more difficult to create and may result in larger weight values.
...
The top 4 ranked input features V26, V8, V31, and V50.
...
```{r, solution_02f}
###
top_4_inputs <- c("V26", "V8", "V31", "V50")
variable_names <- names(Sonar)
variable_names <- variable_names[1:60]

viz_input_grid <- make_test_input_grid(variable_names, top_4_inputs, Sonar)
dim(viz_input_grid)
```
...
viz_input_grid %>%
  ...
  ggplot(aes(x = V26, y = V8)) +
  geom_raster(aes(fill = pred_class)) +
  facet_grid(V50 ~ V31) +
...
The classification surface of the elastic net model has horizontal lines instead of the step changes in the neural network model visualizations to indicate changes in important variables.
...
`mean(pred_prob_nnet$M)` yields mean probability of the `M` class
...
```{r, solution_03f}
bind_cols(pred_prob_nnet, viz_input_grid) %>%
  ggplot(aes(x = V26, y = V8)) +
  geom_raster(aes(fill = M)) + 
  facet_grid(V50 ~ V31) +
  scale_fill_gradient2(mid = 'white', midpoint = 0.5)
```
...
no, random forest does not require preprocessing

the best value for mtry was 2. 

This is because the split value of 2 reduced the variance in the result of the predictive performance by injecting randomness into the treegrowing process. More trees are built and less splits were made, generating better results.
...
No, according to the rf model, the most important variables are V11, V12, V49, and V3, which was not what we found in the neural network model.
...
```{r, solution_04d}
top_4_inputs <- c("V11", "V12", "V49", "V36")
variable_names <- names(Sonar)
variable_names <- variable_names[1:60]
viz_input_grid_rf <- make_test_input_grid(variable_names, top_4_inputs, Sonar)
```
...
Or are we meant to use:
```{r, solution_04f_a}
bind_cols(pred_prob_rf_on_nnet_grid, viz_input_grid) %>%
  ggplot(aes(x = V26, y = V8)) +
  geom_raster(aes(fill = M)) + 
  facet_grid(V50 ~ V31) +
  scale_fill_gradient2(mid = 'white', midpoint = 0.5)
...
```{r, solution_04f_b}
###
bind_cols(pred_prob_rf_on_rf_grid, viz_input_grid_rf) %>%
  ggplot(aes(x = V11, y = V12)) +
  geom_raster(aes(fill = M)) + 
  facet_grid(V36 ~ V49) +
  scale_fill_gradient2(mid = 'white', midpoint = 0.5)
```
...
Based on the visualizations, the best model is the random forest because it has high ROC, sensitivity.
...
Yes, the ROC curves agree with previous cross-validation summary statistics for the models.
