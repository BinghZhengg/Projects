---
title: "CS 1675 Spring 2022 Homework: 03"
...
```
Yes event encompassed approximately half of the observations.
### 1b)
...
```{r, solution_01e_02}
acc_results%>%summary(metric = "Accuracy")
```
...
Best model is model 2 according to accuracy using the simplicity model, further evaluations are needed. 
...
Best model is 2, given the ROC AUC and the simplicity rule. We can be more confident given that this matches the evaluation based on the Accuracy.
...
```
Model 3 has highest TPR, model 8 has highest FPR.
...
```{r, solution_02e}
mod_1_roc$pred %>% yardstick::roc_curve(obs, event)
```


What do you think?  

The sensitivity is the rate that the event is correctly predicted out of all observed events. Since a low threshold will be more likely predict that an event will occur, as it is less discriminatory towards smaller probability values, it will also correlate to a small false negative value. Thus, the equation of TP/(TP+FN) will eventually be TP/TP when the threshold is small enough, equalling to 1.
...
```{r, solution_02f}
mod_1_roc$pred %>% yardstick::roc_curve(obs, event) %>% autoplot()
```
...
The best models is 3 and 2, giving similar curves, but given the simplicity rule, 2 would be better. This is consistent to the ROC AUC evaluations calculated previously. The closest to a completely ineffective model is 8, as it follows a linear trend that indicates threshold variability does not affect the FPR and decreasing the thresholds also does not benefit the TPR when it should. This model will not reflect the reality of the probability no matter at what threshold, thus ineffective.
...
```{r, read_hw_test_set}
hw03_test_url <- 'hw03_test_data.csv'
...
The probability of y=="event" + y=="non_event" = 1.
...
```{r, solution_03c}
test_df_1_b<- test_df_1 %>% mutate(pred_bins = cut(event, 
...
```{r, solution_03d}
test_df_1_b%>% count(pred_bins)
```

There are 10 unique values for each bin.
...
```{r, solution_03e}
test_df_1_b%>%
  ggplot(mapping = aes(x= pred_bins)) +
  geom_bar()
```
...
test_df_1_b%>%
  ggplot(mapping = aes(x= pred_bins)) +
  geom_bar(mapping = aes(fill = y=="event")) +
  scale_color_brewer(palette = "Set1")
```
Bin  0.9,1 consists of only events, but one consists of only non-events.
...
test_df_1_b%>%
  ggplot(mapping = aes(x= pred_bins)) +
  geom_bar(mapping = aes(fill = y=="event"), position = "fill") +
  scale_color_brewer(palette = "Set1")
```

Yes the empirical proportions are roughly equivalent to those of the predicted probability of model 1, although the first half of the proportions do not folloes the trends as expected.
...
test_df_1_c<-test_df_1_b

my_calcurve_1<- test_df_1_c %>%
  mutate(pred_class = ifelse(y=="event", TRUE, FALSE))%>%
  group_by(pred_bins, pred_class) %>%
  summarise(n = n()) %>%
  mutate(prop_event = n / sum(n))

#filter(pred_class=="TRUE")
 
value_calc<-test_df_1_b%>%
  mutate(pred_class = ifelse(y=="event", TRUE, FALSE))%>%
  group_by(pred_bins, pred_class)%>%
  mutate(mid_bin = median(event, na.rm = F))%>%
  count(mid_bin)

temp <- value_calc$mid_bin%>% tibble::as_tibble()

my_calcurve_1 <-my_calcurve_1%>%
  bind_cols(mid_bin = temp)%>%
  mutate(mid_bin = value)

my_calcurve_1 <-  my_calcurve_1%>%filter(pred_class==TRUE)%>%glimpse()
```
...
```{r, solution_04d}
test_df_3_b<- test_df_3 %>% mutate(pred_bins = cut(event, 
                                                   breaks = seq(0, 1, by = 0.1),
                                                   include.lowest = TRUE))

test_df_8_b<- test_df_8 %>% mutate(pred_bins = cut(event, 
                                                   breaks = seq(0, 1, by = 0.05),
                                                   include.lowest = TRUE))
test_df_8_b%>%count(pred_bins)
...
test_df_3_c<-test_df_3_b

my_calcurve_3<- test_df_3_c %>%
  mutate(pred_class = ifelse(y=="event", TRUE, FALSE))%>%
  group_by(pred_bins, pred_class) %>%
  summarise(n = n()) %>%
  mutate(prop_event = n / sum(n))
...
my_calcurve_3 <-  my_calcurve_3%>%filter(pred_class==TRUE)%>%glimpse()
```  
...
test_df_8_c<-test_df_8_b

my_calcurve_8<- test_df_8_c %>%
  mutate(pred_class = ifelse(y=="event", TRUE, FALSE))%>%
  group_by(pred_bins, pred_class) %>%
  summarise(n = n()) %>%
  mutate(prop_event = n / sum(n))
...
my_calcurve_8 <-  my_calcurve_8%>%filter(pred_class==TRUE)%>%glimpse()
...
Model 3 is the most well calibrated model as it follows the trend line the closest, it is obvious given that the mid bins are the data points which are much close to the trend line than the other models. Given the same amount of data points, the obvious aspects of performance can be inferred by the distance of the data points to the line. Additionally, considering the differing sample size in each bin, large deviations from the trend line can be traced to smaller samples within the bin that deviated. For example in model 3, given that the bin around 0.3 to 0.4 is much smaller, thus any data deviating from the trend line can be traced to the fact that this bin has a smaller sample.
...
caret::calibration(y ~ event, data = test_df_1, cuts=10)%>%
  xyplot()
```
Yes, the curve is consistent with the manual calibration curve made above.
