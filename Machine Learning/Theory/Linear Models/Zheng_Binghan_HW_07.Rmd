---
title: "CS 1675 Spring 2022 Homework: 07"
...
Based on the figure, the input and response have a linear relatioship.
...
Given that the there is only one unique maximum in a linear model, we should not be concerned with the initial guess impacting the posterior results. The mode will not change regardless of the value of the initial guess.
...
```{r, solution_01d_c}
###
sd_B0 <- sqrt(laplace_01$var_matrix[1,1])
sd_B1 <- sqrt(laplace_01$var_matrix[2,2])
std_dev_varphi <- sqrt(laplace_01$var_matrix[3,3])
...
###
post_samples_01 %>% 
  ggplot(mapping = aes(beta_01)) +
  geom_histogram(bins = 55)+
  theme_bw()

print(paste( 1-pnorm(0, mean = mean(post_samples_01$beta_01), sd(post_samples_01$beta_01))))
```
...
### 
Ymatdim<- dim(post_pred_samples_01$Ymat)
Ymatdim
```
The dimentsions of Umat and Ymat are both 21 by 2500, the number of columns is 2500, which correlates to the number of prediction points.
...
We are considering the mean trend because the predicted response value contains too much uncertainty to adequately evaluate the quality of the model. We are not concerned with the absolute precision, the error calculations would merely describe the difference between the observed and associated predicted value.
...
error_01_post_01 <- c(train_01$y - post_pred_samples_01$Umat[,1])


#error_01_post_01
#post_pred_samples_01[1]
 #train_01$y - make_post_lm_pred(Xmat_01, post_samples_01[1,])$Umat


### error of the second posterior sample
error_01_post_02 <- c(train_01$y - post_pred_samples_01$Umat[,2])

### error of the third posterior sample
error_01_post_03 <- c(train_01$y - post_pred_samples_01$Umat[,3])
```
...
mae_01_post_03
```
The length of the error_01_post_01 vector is 21. When calculating the MAE we are averaging over the length of the vector, which is the same as the number of values per sample. The three MAE values are different, given that there are different values made within each sample, the average will be different, and thus difference between a variable mean and a set observed value.
...
the dimensions are 21 by 2500.

```{r, solution_02e, eval=TRUE}
absE01mat <- abs(post_pred_samples_01$Umat[,]-as.numeric(train_01$y))

dimension<-dim(absE01mat)
...
### 
quantile <- quantile(MAE_01)
quantile

```

The MAE is uncertain because it does not take into account the uncertainty around the mean. Therefore, when taking the difference between the mean trend and the observed values, which is the MAE, the uncertainty is carried over making the MAE uncertain as well.
...
```{r solution_03a}
###
train_02 %>% ggplot(mapping = aes(x=x, y=y))+
  geom_point() +
  geom_smooth(formula =  y~x, method = lm, color = "blue")+
  geom_point(data = test_02, aes(x=x, y=y, color = "orange"))+
  geom_smooth(data = test_02, formula =  y~x, method = lm, color = "red")
...
There are 150 rows in the training matrices and 50 rows in the test matrices.
...
make_spline_basis_mats <- function(J, train_data, test_data)
{
  ...
  train_matrix <- model.matrix(train_data$y ~ splines::ns(train_data$x, knots = knots_use_basis), 
                               data =train_data)
  
  test_matrix <- model.matrix(test_data$y~ splines::ns(test_data$x, knots = knots_use_basis), 
                              data = test_data )
...
#spline_matrices$design_matrix[[1]]
...
calc_mae_from_laplace <- function(mvn_result, Xtrain, Xtest, y_train, y_test, num_samples)
{
  ...
  error_train <- pred_train$Umat - as.numeric(y_train)
  
  # calculate the error between the test set predictions
  # and the test set observations
  error_test <- pred_test$Umat - as.numeric(y_test)
...
set.seed(52133)
all_spline_mae_results <- purrr::pmap_dfr(list(all_spline_models,                 spline_matrices$design_matrix,                                  spline_matrices$test_matrix),calc_mae_from_laplace,
                            y_train = train_02$y,
                            y_test = test_02$y,
                            num_samples = 2500)
...
### 
all_spline_mae_results %>% filter(J > 1) %>% ggplot(mapping = aes(x = J, y = mae)) + stat_summary(geom = "line", fun = "median") + facet_wrap(~dataset)

```

Based on the visualization, it seems that starting from 13th degree of freedom, the overlap between the trainning and data set consistently only overlap in data points considered as outliers. Additionally, given that the MAE in the training set becomes closer to 0 and remaining constant, it is safe to assert that the models beyond df=13 (model 6) are overfit. Model 5 may also be overfit.
...
Based on the visualization, the evaluation penalized the overfit models based on the resulting coefficient values, indicated that model 4 and 5 to be the only viable models. Model 4 is evaluated to be the relatively better model with 7 degrees of freedom. 
...
The data split comparison gave a visualization of the result of the training and test sets. The relative coefficients that will later become the source of penalization are effectively visualized in the evidence based assessment, where we can observe the impacts of the measured coefficients from the data split. Overall, they alligned by indicating that model 4 seemed to be the most reasonable model. While model 5 seemed as if it was overfit, the calculations made a definitive evalutation that the coefficients would not result in as large a penalization as the further models. 
