...
```{r, read_prob_01_data}
url_01 <- 'hw_10_prob_01_train.csv'
...
The first row represent the hidden unit parameters, the second row is beta 1 for each hidden unit. The alpha vector consist of 3 elements because there are two hidden units, and the third element would be the scalar intercept.
...
The hidden linear predictors relative to the beta parameter values both follow a positive trend for the first hidden linear predictor and a negative trend for the second hidden linear predictor.
...
$$
\mathbf{f} = a_{0} + \mathbf{H}\boldsymbol{\alpha}
$$
...
The neural network fit an approximate s curve. The interval of approximately -1 to positive 1 for the x value were approximate well as the plotted observation values alligned incredibly close to the fit.
...
Hidden linear predictors: h_01 from the previous problem had a positive slope and h_01 from this problem contained a negative slope. h_02 from both problems contained negative slopes. 

Hidden non-linear predictors: h_01 has a negative slope in this problem but in the previous problem h_01 contains a positive slope. h_02 from this problem and the previous have the same trend(s-curve).

The reason as to why these trends are non entirely consistent, is because the initial values of the beta parameters are different for this problem. This of course will change the relationships within the hidden unit linear values. h_02 contained the same trends for the non-linear units because their corresponding hidden linear values were the same.

This difference observed in the visualization would  still not result in a different neural network than the one made before. Slight deviations int the fit may occur, but not enough where it would alter the fit significantly.
...
They both exhibit the same behavior and have basically identical slopes. 
...
Since we do not have the output layer parameters we cannot entirely explain the neural network behavior with just the information on the weights acting on the inputs. TO describe neural network behavior, we need both the hidden linear predictors and the output layer parameters.
...
What do you think?  
In model A, the fifth hidden parameter has a positive trend in the linear and non-linear values, whereas both the linear and nonlinear values for this parameter offers a negative trend in 5b.
...
results_5hidden %>% 
  ...
  theme_bw()
```
Both models are highly alligned with the true sine wave.
...
The model with 5 hidden units is performing better. The complexity is generally controlled by the number of parameters and the number of beta values associated. In this case, there are beta values for the hidden parameters, thus the neural network model complexity is controlled by the number of hidden parameters. Tuning the model would require adjusting the number of parameters used to construct the model.
...
The estimate on the slope is 1.
...
There are 2 unknowns in the linear model with the sine wave basis function, and there were 16 unknowns parameters in the neural network with 5 hidden units.
...
The neural network can provide a fit that is better at interpreting the data by offering a basis function without having to specify it as we do in the linear model. In this case, the linear model was simple since we specified the sine function, but without knowing that the data would most allign with this basis function, it would be difficult to find a good basis function just from guessing.
...
```
x1 and x2 seems to offer a positive trend based on their distribution, and x3 offers a negative trend.
...
tibble::tibble(
  ...
  theme(legend.position = "top") 
```
The logistic result and the tangent function both exhibit an s curve and both curves adopt a median at x = 0. However, since the logistic function is log transformed, all the plotted values in that function are positive, whereas the tangent function has a greate range of the value since it does not have to be positive.
...
$$
f = \alpha_0 \space + \space \mathbf{H} \alpha
$$

$$
\Sigma_{n=1}^{N} \left( y - f    \right)^{2}
$$
...
The starting values impact the optimal parameters approximated ina neural networkd model because the neural network likelihood surface is multimodal. Depending on the location of the starting guesses, the approximation will find the variety of available optimas on the surface.
...
set.seed(412412)
init_guess_03_c <- rnorm(info_three_units$num_params, 0, 1)
  
set.seed(214214)
init_guess_03_d <- rnorm(info_three_units$num_params, 0, 1)
```
...
All of the models except for model a are equally good given the same SSE values within optim().
...
According to the results, the best model would be the model with 24 hidden unit neural networks using parameters a since it has the smallest SSE values.
...
```{r, eval=TRUE}
my_neuralnet_mse <- function(theta, num_hidden, tanh, X, y)
{
  # extract the hidden unit parameters
  length_beta_per_unit <-ncol(X) # how many betas are there???????
  total_num_betas <- num_hidden*length_beta_per_unit# how many total betas are there???????
...
{
  my_neuralnet_mse(theta, num_hidden, tanh, X, y)
}
```
...
set.seed(1234)
num_hidden_units <- 3
num_parameters <- num_hidden_units * (ncol(Xtest_03)) + num_hidden_units + 1
THETA <- rnorm(num_parameters,0,1)
mse_result_3 <- assess_nnet_mse(THETA, num_hidden_units, tanh, Xtest_03, prob_03_df$y)

set.seed(1234)
num_hidden_units <- 6
num_parameters <- num_hidden_units * (ncol(Xtest_03)) + num_hidden_units + 1
THETA <- rnorm(num_parameters,0,1)
mse_result_6 <- assess_nnet_mse(THETA, num_hidden_units, tanh, Xtest_03, prob_03_df$y)

set.seed(1234)
num_hidden_units <- 12
num_parameters <- num_hidden_units * (ncol(Xtest_03)) + num_hidden_units + 1
THETA <- rnorm(num_parameters,0,1)
mse_result_12 <- assess_nnet_mse(THETA, num_hidden_units, tanh, Xtest_03, prob_03_df$y)

set.seed(1234)
num_hidden_units <- 24
num_parameters <- num_hidden_units * (ncol(Xtest_03)) + num_hidden_units + 1
THETA <- rnorm(num_parameters,0,1)
mse_result_24 <- assess_nnet_mse(THETA, num_hidden_units, tanh, Xtest_03, prob_03_df$y)

print(paste("MSE - Number Hidden Units 3: ", mse_result_3))
print(paste("MSE - Number Hidden Units 6: ", mse_result_6))
print(paste("MSE - Number Hidden Units 12: ", mse_result_12))
print(paste("MSE - Number Hidden Units 24: ", mse_result_24))
...
According to the MSE output, the best model is the model with three hidden units as it has the lowest MSE value.
...
set.seed(412412)
nnet_default <- caret::train( y ~ x1 + x2 + x3,
                    data = prob_03_test_df,
                   method = "nnet",
                   metric = my_metric,
...
```
 ...
 Tuning parameter where size of 5 and decay of 1e-04 is best.
...
The size and decay tuning parameters limits overfitting in the tuning grid. The decay parameter corresponds to the weight decay and size is the number of hidden parameters. 