---
title: "CS 1675 Spring 2022 Homework: 06"
...
print(paste("Sample Average: ", sample_avg))

#Sample Std Deviation
sample_std <- sd(hw06_df$x)
...
$$
\tau_{0} = \frac{12-10}{4} =0.5
$$

$$
\mu_{0} = \frac{12+10}{2} =11
$$
```{r}
my_tau_0 <- ((12-10)/4)
my_mu_0 <- ((10+12)/2)
...
```{r, solution_01d_b}
### 
check1 <-my_logpost(c(13, 5), hw06_info)
check1
```
...
```{r, solution_01d_c}
### 
check2<- my_logpost(c(7, 1.5), hw06_info)
check2
```
...
mu_grid_lwr <- (-3)*(hw06_info$tau_0)+ hw06_info$mu_0
mu_grid_upr <- (3)*(hw06_info$tau_0)+ hw06_info$mu_0
...
```{r, solution_01f_b}
### 
check3<- eval_logpost(13, 5, my_logpost, hw06_info)
check3
```
...
What do you think?  
Our posterior has been restricted for mu and sigma as such is the reason as to why the posterior mode does not lie on the top of the sample average for mu and sigma. While the posterior mode has slightly shifted towards the left from our priors our prior hyperparameters are the restrictive force behind keeping our posterior mode closer to our initial values for mu and tau defined in hw06_info.
...
$$
p\left(\mu \mid \mathbf{x}, \sigma\right) \propto \log \left( \prod_{n=1}^{N} \exp \left( - \frac{1}{2 \sigma^2}\left(   x_n - \mu \right)^2 \right) \times \exp\left(  - \frac{1}{2 \tau_{0}^2}(\mu -\mu_0)^2\right) \right)
$$


$$
\log\left(p\left(\mu \mid \mathbf{x}, \sigma\right) \right) \propto - \frac{1}{2 \sigma^2} \Sigma_{n=1}^{N} \left(   x_n - \mu \right)^2  - \frac{1}{2 \tau_{0}^2}(\mu -\mu_0)^2 
$$
$$
f(u) = - \frac{1}{2 \sigma^2} \Sigma_{n=1}^{N} \left(   x_n - \mu \right)^2  - \frac{1}{2 \tau_{0}^2}(\mu -\mu_0)^2
$$

$$
f'(u) =  \frac{1}{ \sigma^2} \Sigma_{n=1}^{N} \left(   x_n - \mu \right)  - \frac{1}{ \tau_{0}^2}(\mu -\mu_0)
$$


$$
f'(u) =  \frac{1}{ \sigma^2}\left( \Sigma_{n=1}^{N} \left(   x_n  \right) - \Sigma_{n=1}^{N} \left(   \mu \right) \right)- \frac{1}{ \tau_{0}^2}(\mu -\mu_0)
$$

$$
f'(u) =  \frac{N \bar{x}}{\sigma^2}  -  \frac{\mu N}{\sigma^2}  - \frac{\mu -\mu_0}{ \tau_{0}^2}
$$

$$
f''(u) =  -\frac{\mu N}{\sigma^2} - \frac{\mu}{ \tau_{0}^2}
$$

$$
f''(u) =  - \left(\frac{ N}{\sigma^2} + \frac{1}{ \tau_{0}^2}  \right)\\
$$
### 2b)
...
What do you think? 
With the first order derivative, the slope by itself when zero may indicate the max or min and when associated with the second order derivative(Hessian).This indicates the curvature of the surface, if the curvature in every direction points downward/negative then the mode corresponds to the maximum log-posterior. 
The second order derivative gives the curvature around the mode, and if the slope on all sides are negative, it indicates that this is the maximum and not the minimum log-posterior density.
...
$$
-\frac{1}{\tau_{N}^2} =  - \left(\frac{ N}{\sigma^2} + \frac{1}{ \tau_{0}^2}  \right)
$$

$$
-\tau_{N}^2 = -\left(\frac{ N}{\sigma^2} + \frac{1}{ \tau_{0}^2}  \right)^{-1}
$$

$$
\tau_{N}^2 = -\left(-\left(\frac{ N}{\sigma^2} + \frac{1}{ \tau_{0}^2}  \right)^{-1} \right)
$$

$$
\tau_{N} =  -\sqrt{- \left(\frac{ N}{\sigma^2} + \frac{1}{ \tau_{0}^2}  \right)^{-1} } \\
$$
### 2d)
...
As the sigma is given, we no longer have to approximate based on the transformation of 2 unknown variables as was discussed in the example given in class. Since sigma is given, the posterior mean and posterior standard deviation can both be calculated with the values given, rendering it no longer an approximation but the specific application.
...
Setting bounds on sigma prevents the Gaussian distribution given within the approximation from considering sigma values that are negative since the distribution allows for a range between negative infinity and positive infinity. In application, the post transformation lower and upper bound when given natural log would assure that the bounds respect the natural lower bound of 0.
...
$$
p \left( \varphi \mid \mathbf{x}, \mu\right) \propto \prod_{n=1}^{N} \mathrm{normal}\left( x_n \mid \mu, g^{-1} \left( \varphi  \right) \right)*  \mathrm{normal}\left(\mu \mid \mu_{0}, \tau_{0}\right)  *  \mathrm{Exp}\left(\ g^{-1} \left( \varphi \right) \mid \lambda \right) * \left| \frac{d}{d\varphi} \left( g^{-1}\left(\varphi\right) \right) \right|
$$

$$
\log(p \left( \varphi \mid \mathbf{x}, \mu\right)) \propto \Sigma_{n=1}^{N} \log\left( \mathrm{normal}\left( x_n \mid \mu, g^{-1} \left( \varphi  \right)  \right)*  \mathrm{normal}\left(\mu \mid \mu_{0}, \tau_{0}\right)  *  \mathrm{Exp}\left(\ g^{-1} \left( \varphi \right) \mid \lambda \right) * \left| \frac{d}{d\varphi} \left( g^{-1}\left(\varphi\right) \right) \right| \right)
$$

$$
\log(p \left( \varphi \mid \mathbf{x}, \mu\right)) \propto \Sigma_{n=1}^{N} \log\left( \mathrm{normal}\left( x_n \mid \mu, g^{-1} \left( \varphi  \right)  \right)\right) + \log \left( \mathrm{normal}\left(\mu \mid \mu_{0}, \tau_{0}\right) \right) + \log( \mathrm{Exp}\left(\ g^{-1} \left( \varphi \right) \mid \lambda \right) * \left| \frac{d}{d\varphi} \left( g^{-1}\left(\varphi\right) \right) \right| 
$$
...
$$
g^{-1}\left(\varphi\right) = \log(\varphi)
$$

Write the log of the derivative adjustment in an equation block. 
$$
\log \left( \frac{d}{d\varphi}\exp(\varphi) \right) = \varphi 
$$
...
```{r, solution_03d_b}
### 
check4<-my_cv_logpost(c(13,0), hw06_info)
check4
```
...
```{r, solution_03d_c}
###
check5<- my_cv_logpost(c(7,-1), hw06_info)
check5
```
...
With our values transformed, the sample average for mu and sigma retain their relative position within our posterior mode given our original parameter space between μ and σ in the previous grid. Contour shape of the transformed log-posterior is much more symmetric and approximate of Normal as opposed to the original log-posterior contour which appeared much more oblong and stretched.
...
Optimal parameter values are the same and the Hessian matrices are identical produced by the different initial guess. The function and gradient values are different. The previous visualizations gives an indication that the relative distance from where the initial guess are positioned from the yellow portion would correlate between the differences of the function and gradient values. As the function and gradient values reflect the path of which optimization takes from the initial guess, the further the guess is away from the mode, the greater the values are for function and gradient as the path would be longer.
...
post_mu <- laplace_result$mode[1]
post_varphi <- laplace_result$mode[2]

std_dev_mu <- sqrt(laplace_result$var_matrix[1,1])
std_dev_varphi <- sqrt(laplace_result$var_matrix[2,2])

correlation_coeff <- laplace_result$var_matrix[1,2]/(std_dev_mu*std_dev_varphi)

print(paste("Posterior Mean mu ", post_mu))
print(paste("Standard Deviation mu: ", std_dev_mu))
print(paste("Posterior Mean varphi: ", post_varphi))
print(paste("Standard Deviation varphi: ", std_dev_varphi))
print(paste("Correlation Coefficient for mu and phi: ", correlation_coeff))
```


Posterior Correlation Coefficient -> 1.15 
...
print(paste("probability that mu is less than sample average:", pnorm(sample_avg, mean = post_mu, sd = std_dev_mu)))
```
...
post_samples<-generate_post_samples(laplace_result, 10^4)
...
Exponential (inverse link function) on the posterior mean results in value slightly less than posterior mean on sigma =3.068.

```{r}
inv_link_sig = exp(1.0933)
inv_link_sig
```
...
post_samples %>% 
  ...
  geom_histogram(bins=55)
```
Yes, distribution does look Gaussian w/ slight right skew.
...
Include as many code chunks and discussion text as you feel are necessary.  
```{r, solution_5e}
print(paste("Probability Sigma greater than 4: ", 1-pnorm(4, mean = 3.068, sd(post_samples$sigma))))
```