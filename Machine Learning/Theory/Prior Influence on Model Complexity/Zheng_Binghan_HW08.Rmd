---
title: "CS 1675 Spring 2022 Homework: 08"
...
Based on the visualization, there should be a relationship between the input and the response that is positive.

```{r, solution_01a}
df %>% ggplot() +
  geom_point(mapping = aes(x = x1, y = y), color = "red") +
  geom_point(mapping = aes(x = x2, y = y), color = "steelblue") +
  geom_smooth(mapping = aes(x = x1, y = y), color = "red") +
  geom_smooth(mapping = aes(x = x2, y = y), color = "steelblue") +
  theme_bw()
```
...
All features within the coefficient plot contain 0 therefore none are statistically significant. 
...
The interaction term (x1:x2) and x1 coefficient confidence intervals do not contain 0, therefore they are statistically significant.

```{r, solution_01c}
### add more code chunks if you like

mod02 <- lm(y ~ x1 + x2 + x1:x2,
               data = df)
mod02 %>% 
  coefplot::coefplot()+
  theme_bw()
...
#### SOLUTION
 ...
 The quadratic term for x2, I(x2^2) and intercept coefficient confidence interval does not contain 0, therefore they are statistically significant.
...
The intercept confidence interval does not contain zero, therefore it is considered statistically significant.
...
None of the features in this coefficient confidence interval plot are statistically significant
...
Model 6 is best according to R-sq.

Model 3 is best according to AIC and BIC
...
The predictions are consistent with the model performance rankings based on AIC and BIC. 

The worst performing models according to the AIC and BIC metrics would be model 1 and model 6 respectively. Model 1's defining characteristic would be  that its intervals are inflexible and thus AIC being considered poor since it was not able to explain accurately much of the variability therefore resulting in a poor(High) AIC score.  The extreme variability due to the large number of features present in the model is the defining characteristic of this worst performing model for BIC(model 6).
...
laplace_03_weak <- my_laplace(seq(0,0, length.out = ncol(X03)+1), lm_logpost, info_03_weak)
  
laplace_06_weak <- my_laplace(seq(0,0, length.out = ncol(X06)+1), lm_logpost, info_06_weak)
laplace_03_weak$converge == "YES"
laplace_06_weak$converge == "YES"
```
...
cols <- ncol(X03)
means03<-laplace_03_weak$mode[1:cols]
sd03<-laplace_03_weak$var_matrix[seq(1,cols)][seq(1,cols)]
names03 <- dimnames(X03)[[2]]
viz_post_coefs(means03,sd03,names03)
```
...
cols <- ncol(X06)
means06<-laplace_06_weak$mode[1:cols]
sd06<-laplace_06_weak$var_matrix[seq(1,cols)][seq(1,cols)]
names06 <- dimnames(X06)[[2]]
viz_post_coefs(means06,sd06,names06)
```
...
```{r, solution_03e}
### add more code chunks if you like
evidence_third <- exp(laplace_03_weak$log_evidence)

evidence_sixth <- exp(laplace_06_weak$log_evidence)

bayes_factor <- evidence_third/evidence_sixth

bayes_factor
```
Model 3 should be doing better since the bayes factor is greater than 1, indicating that the value for evidence for model 3 is much higher than model 6.
...
```{r, solution_03f_b}
laplace_03_strong<- my_laplace(seq(0,0, length.out = ncol(X03)+1), lm_logpost, info_03_strong)

laplace_06_strong<- my_laplace(seq(0,0, length.out = ncol(X06)+1), lm_logpost, info_06_strong)

check <- laplace_03_strong$converge == "YES" && laplace_06_strong$converge=="YES"
...
### add more code chunks if you like
cols <- ncol(X03)
means03strong<-laplace_03_strong$mode[1:cols]
sd03strong<-laplace_03_strong$var_matrix[seq(1,cols)][seq(1,cols)]
names03 <- dimnames(X03)[[2]]
viz_post_coefs(means03strong,sd03strong,names03)

cols <- ncol(X06)
means06strong<-laplace_06_strong$mode[1:cols]
sd06strong<-laplace_06_strong$var_matrix[seq(1,cols)][seq(1,cols)]
names06 <- dimnames(X06)[[2]]
viz_post_coefs(means06strong,sd06strong,names06)
```
...
```{r, solution_03h_b}
laplace_03_very_strong<- my_laplace(seq(0,0, length.out = ncol(X03)+1), lm_logpost, info_03_very_strong)

laplace_06_very_strong<- my_laplace(seq(0,0, length.out = ncol(X06)+1), lm_logpost, info_06_very_strong)

check <- laplace_03_very_strong$converge == "YES" && laplace_06_very_strong$converge=="YES"
...
### add more code chunks if you like
cols <- ncol(X03)
means03strong<-laplace_03_very_strong$mode[1:cols]
sd03strong<-laplace_03_very_strong$var_matrix[seq(1,cols)][seq(1,cols)]
names03 <- dimnames(X03)[[2]]
viz_post_coefs(means03strong,sd03strong,names03)

cols <- ncol(X06)
means06strong<-laplace_06_very_strong$mode[1:cols]
sd06strong<-laplace_06_very_strong$var_matrix[seq(1,cols)][seq(1,cols)]
names06 <- dimnames(X06)[[2]]
viz_post_coefs(means06strong,sd06strong,names06)
```
...
The prior standard deviation restricted the confidence interval on the regression coefficients more as the strength of the priors increased on each of the Beta coefficients. The resulting confidence intervals appear to making precise estimations. When using a weak prior, the confidence intervals were allowed to take on a much wider range of values. Utilizing stronger priors resulted in smaller ranges for the coefficient confidence intervals
...
The prior does influence the evaluation of the model. When using a strong prior, the Bayes factor indicates the third model is  more likely the better model by a significant amount.When further increasing the strength of the prior, the bayes factor calculation then indicates the sixth model is more likely the better model. 

```{r, solution_03k}
### add more code chunks if you like\
evidence_third <- exp(laplace_03_strong$log_evidence)

evidence_sixth <- exp(laplace_06_strong$log_evidence)

bayes_factor_strong <- evidence_third/evidence_sixth

bayes_factor_strong


evidence_third <- exp(laplace_03_very_strong$log_evidence)

evidence_sixth <- exp(laplace_06_very_strong$log_evidence)

bayes_factor_very_strong <- evidence_third/evidence_sixth
...
viz_grid<- viz_grid %>% 
  mutate(y = (splines::ns(x1, df = 12) * (x2 +I(x2^2)+I(x2^3)+I(x2^4))))

X06_test<- model.matrix(y~splines::ns(x1, df = 12) * (x2 +I(x2^2)+I(x2^3)+I(x2^4)), data = viz_grid)
...
post_pred_summary_viz_06_very_strong<-post_pred_summary_viz_06_very_strong %>% 
  left_join(viz_grid %>% tibble::rowid_to_column("pred_id"),
            by = 'pred_id')

post_pred_summary_viz_06_very_strong %>% 
    ggplot(mapping = aes(x = x1))+
  geom_ribbon(aes(ymin = mu_lwr, ymax = mu_upr), fill = "orange")+
  geom_ribbon(aes(ymin = y_lwr, ymax = y_upr), fill = "grey")+
  geom_line( y = post_pred_summary_viz_06_very_strong$mu_avg)+
  facet_wrap(facets = post_pred_summary_viz_06_very_strong$x2)+
  coord_cartesian(ylim=c(-7,7))
```
...
The posterior predictions are consistent with the behavior of the posterior coefficients.  As prior standard deviation decreased, the interval for the predictions would more so closely adopt the same trend as the confidence interval. Both became more linear and restricted. 
...
X06_glmnet <- model.matrix(y~splines::ns(x1, df = 12) * (x2 +I(x2^2)+I(x2^3)+I(x2^4)), data = df)
...
```{r, solution_05d}
### add more code chunks if you like
lasso_03_cv_tune <- cv.glmnet(X03_glmnet, df$y, nfolds = 10,alpha = 1)
lasso_06_cv_tune <- cv.glmnet(X06_glmnet, df$y, nfolds = 10,alpha = 1)
```
...
After tuning and via the one-standard error rule, the evaluation prefers simpler models, leaving one coefficient remaining after tuning.
...
The final tuned model 3 and model 6 both contained I(x2^2) and the intercept, which are the three not turned off by the Lasso penalty. The coefficient values differ regardless.

```{r, solution_05f}
### add more code chunks if you like
a<-coef(lasso_03_cv_tune)
b<-coef(lasso_03_cv_tune)
